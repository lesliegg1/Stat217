...
\begin{center}
\large{STAT 217: Multicollinearity and VIFs $4/17$} \\
\end{center}
...
\item Interpret the estimate for the coefficient on income in the linear model of happiness on income and caffeine.
\vspace{1in}
...
\item Use the following plots to assess the assumptions for the multiple linear regression model.

<<pairs, echo=FALSE, message=FALSE, out.width="\\linewidth", fig.width=10>>=
require(mosaic)
require(psych)
...
\item What was the standard error for the coefficient on income in the simple linear regression model?
\vspace{0.5in}

\item What is the standard error for the coefficient on income when both caffeine and income are in the model?
\vspace{0.5in}

\item Based off the two previous questions, calculate $\sqrt{VIF}$ for the coefficient on income. Then, find $VIF_{caffeine}$.
\vspace{0.75in}

\item Below is the VIF calculated by R. Does this match up, roughly, with what you found? There may be some rounding error.
<<vifs,echo=TRUE>>=
require(car)
vif(ic.lm)[2]
@
