...
\item (Old Faithful): Old Faithful Geyser in Yellowstone National Park derives its name and fame from the regularity (and beauty) of its eruptions. Rangers usually post the predicted times of eruptions for visitors. R. A. Hutchinson, a park geologist, collected measurements of the eruption durations (in minutes) and the subsequent time intervals before the next eruption (in minutes) over an 8-day period. Help rangers use the data to explain the relationship between duration and subsequent time to the next eruption.
...
\item they are obtained by squaring the smallest estimated regression coefficients
