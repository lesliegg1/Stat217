...
\begin{center}
\large{STAT 217: Unreplicated designs 9/30} \\
\end{center}
...
\item Below is the interaction ANOVA table. Note the warning. What do you notice about the F-statistic and the p-value?
<<adhesives, echo=FALSE, comment="">>=
lm.int <- lm(strength~pressure*temperature, data=adhesive)
anova(lm.int)
@
...
intplot(strength~pressure*temperature, data=adhesive)
@
